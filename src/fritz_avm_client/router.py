...
class RouterClient:
    ...
    def get_dsl_stats(self) -> DslStats:
        ...
        try:
            ...
            return DslStats(
                downstream_attenuation=float(attenuation[0]) if attenuation and attenuation[0] is not None else None,
                upstream_attenuation=float(attenuation[1]) if attenuation and len(attenuation) > 1 and attenuation[1] is not None else None,
                downstream_noise_margin=float(noise_margin[0]) if noise_margin and noise_margin[0] is not None else None,
                upstream_noise_margin=float(noise_margin[1]) if noise_margin and len(noise_margin) > 1 and noise_margin[1] is not None else None,
            )
        except Exception:
            return DslStats()
